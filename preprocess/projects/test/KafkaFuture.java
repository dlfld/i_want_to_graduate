 ...
 * If converting {@link KafkaFuture#whenComplete(BiConsumer)} or
 * {@link KafkaFuture#thenApply(BaseFunction)} to
...
public abstract class KafkaFuture<T> implements Future<T> {
    ...
    /**
     * Returns a new KafkaFuture that is completed when all the given futures have
     * completed. If
     * any future throws an exception, the returned future returns it. If multiple
     * futures throw
    ...
    public static KafkaFuture<Void> allOf(KafkaFuture<?>... futures) {
        ...
        CompletableFuture.allOf(
                Arrays.stream(futures)
                        .map(kafkaFuture -> {
                            // Safe since KafkaFuture's only subclass is KafkaFuture for which
                            // toCompletionStage()
                            // always return a CF.
    ...
    /**
     * Gets a {@code CompletionStage} with the same completion properties as this
     * {@code KafkaFuture}.
     * The returned instance will complete when this future completes and in the
     * same way
     * (with the same result or exception).
     *
     * <p>
     * Calling {@code toCompletableFuture()} on the returned instance will yield a
     * {@code CompletableFuture},
     * but invocation of the completion methods ({@code complete()} and other
     * methods in the {@code complete*()}
     * and {@code obtrude*()} families) on that {@code CompletableFuture} instance
     * will result in
     * {@code UnsupportedOperationException} being thrown. Unlike a "minimal"
     * {@code CompletableFuture},
     * the {@code get*()} and other methods of {@code CompletableFuture} that are
     * not inherited from
     * {@code CompletionStage} will work normally.
     *
     * <p>
     * If you want to block on the completion of a KafkaFuture you should use
     * {@link #get()}, {@link #get(long, TimeUnit)} or {@link #getNow(Object)},
     * rather then calling
     ...
     *
     * The function may be invoked by the thread that calls {@code thenApply} or it
     * may be invoked by the thread that
     * completes the future.
     */
    public abstract <R> KafkaFuture<R> thenApply(BaseFunction<T, R> function);

    /**
     * Prefer {@link KafkaFuture#thenApply(BaseFunction)} as this function is here
    ...
    /**
     * Returns a new KafkaFuture with the same result or exception as this future,
     * that executes the given action
     * when this future completes.
     *
     * When this future is done, the given action is invoked with the result (or
     * null if none) and the exception
     * (or null if none) of this future as arguments.
     *
     * The returned future is completed when the action returns.
     * The supplied action should not throw an exception. However, if it does, the
     * following rules apply:
     * if this future completed normally but the supplied action throws an
     * exception, then the returned future completes
     * exceptionally with the supplied action's exception.
     * Or, if this future completed exceptionally and the supplied action throws an
     * exception, then the returned future
     * completes exceptionally with this future's exception.
     *
     * The action may be invoked by the thread that calls {@code whenComplete} or it
     * may be invoked by the thread that
     * completes the future.
     *
    ...
    /**
     * Returns true if this CompletableFuture was cancelled before it completed
     * normally.
     */
    @Override
